...
const EventSlider = ({ events }) => {
  ...
  const goToEvent = (event) => {
    console.log(event);
    navigate("/event", { state: { event, booked: event.booked } });
  };
    ...
    <Container>
      ...
      >
        {events //to check if the events are loaded
          ? events
              ...
              .map((event) => (
                <Card key={event.id} onClick={() => goToEvent(event)}>
                  <img
                    src="https://images.hdqwalls.com/wallpapers/water-through-rocks-4k-kl.jpg"
                    alt="eventImage"
                  />
                  <h3>{event.name}</h3>
                  <div>
          ...
          : noEvents.map((events) => <LoadingCard></LoadingCard>)}
      </CustomCarousel>
    </Container>
  );
};
